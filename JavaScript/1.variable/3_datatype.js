...
//3. 문자형
let name = "홍길동";
console.log("문자형 -> ", name); //---는 name은 변수 말고도 기능으로 사용되기도 하여 권장하지 않는다는 뜻
...
console.log(typeof x == typeof xx); //값만 비교
console.log(typeof x === typeof xx); //값+데이터타입 비교
console.log(typeof x == typeof y); //값만 비교
console.log(typeof x === typeof y); //값+데이터타입 비교
console.log(x == y); //typeof를 빼면 true로 취급
console.log(x === y); //===로 타입까지 체크해서 false로 취급
