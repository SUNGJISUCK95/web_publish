...
let result2 = filter(['홍길동','이순신','홍길동','이순신']);
...
function textFilter(array) {
    ...
    return array.filter((text) => text.length >= 3); //새로운 배열 생성
}

//textList를 입력받아, 3글자 이상이 아닌 text이면 새로운 배열 객체를 생서아여 반환
function textFilter(array) {
...
let result3 = textFilter2(['javascript', 'java', 'css', 'ab', 'html']);
console.log(result);
console.log(result3);
